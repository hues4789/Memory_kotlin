...
class RandomLoginEspressoTest {
    @Rule
    @JvmField
    var mActivityTestRule = ActivityTestRule(RandomLoginActivity::class.java)

    @Test
    fun fromNumEmptyError(){
        Espresso.onView(ViewMatchers.withId(R.id.FromNum)).perform(ReplaceTextAction(""))
        Espresso.onView(ViewMatchers.withId(R.id.btRandomStart)).perform(ViewActions.click())
